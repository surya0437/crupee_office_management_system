...
$sqltran = mysqli_query($con, "SELECT n.task_id, n.task_writer, n.comment_status, n.task_title, n.task_phase, n.task_status, n.date, n.task_progress, n.ref_needed, n.issue_recorded, n.title_desc, e.department, e.designation, e.first_name AS fname, e.last_name AS lname FROM wy_tasks n INNER JOIN wy_users e ON n.task_writer = e.emp_code ORDER BY n.task_id DESC")or die(mysqli_error($con));
 $arrVal = array();

//For total Task Count 
 $sqltran_count = mysqli_query($con, "SELECT COUNT(task_id) AS total_tasks FROM wy_tasks")or die(mysqli_error($con));
...
else 
{
 ...
 'designation'=> $rowList['designation'],
...
'view'=> '<button type="button" name="delete" id="'.$row_id.'" class="btn btn-outline-danger btn-sm task-delete">Delete</button> <button type="button" name="task_comm" id="'.$row_id.'" class="btn btn-outline-primary btn-sm task_comm">Comment</button>'
 );
